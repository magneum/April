    ...
    async play(song, message) {
        ...
        if (!song) {
            setTimeout(function () {
                ...
                queue.channel.leave();
                queue.textChannel.send("**🦋------------------  𝗡𝗢𝗜𝗥  ------------------🦋**\n\n⚜️**Noir** Leaving voice channel...");
            },
                NOIRWAKE);
            queue.textChannel.send("**🦋------------------  𝗡𝗢𝗜𝗥  ------------------🦋**\n\n⚜️**Noir** ❌ Music queue ended.")
                .catch(console.error);
            ...
            switch (reaction.emoji.name) {
                ...
                case "🤫":
                    ...
                    if (queue.muted) {
                        ...
                        queue.textChannel.send("<@{author}>\n\n**🦋------------------  𝗡𝗢𝗜𝗥  ------------------🦋**\n\n⚜️**Noir** 🤫 Muted the music!",
                            { author: user })
                            .catch(console.error);
                    } else {
                        queue.connection.dispatcher.setVolumeLogarithmic(queue.volume / 100);
                        queue.textChannel.send("<@{author}>\n\n**🦋------------------  𝗡𝗢𝗜𝗥  ------------------🦋**\n\n⚜️**Noir** 🔊 Unmuted the music!",
                            { author: user })
                            .catch(console.error);
                    }
                    break;
                case "🔉":
                    ...
                    queue.connection.dispatcher.setVolumeLogarithmic(queue.volume / 100);
                    queue.textChannel
                        .send("<@{author}>\n\n**🦋------------------  𝗡𝗢𝗜𝗥  ------------------🦋**\n\n⚜️**Noir** 🔉 Decreased the volume, the volume is now {volume}%",
                ...
                case "🔊":
                    ...
                    queue.connection.dispatcher.setVolumeLogarithmic(queue.volume / 100);
                    queue.textChannel.send("<@{author}>\n\n**🦋------------------  𝗡𝗢𝗜𝗥  ------------------🦋**\n\n⚜️**Noir** 🔊 Increased the volume, the volume is now {volume}%", { author: user, volume: queue.volume })
                        .catch(console.error);
                    break;
                case "🔁":
                    reaction.users.remove(user).catch(console.error);
                    ...
                    queue.textChannel.send(`**🦋------------------  𝗡𝗢𝗜𝗥  ------------------🦋**\n\n⚜️**Noir** Loop is now ${queue.loop ? noir_loader.__("common.on") : noir_loader.__("common.off")}`)
                        .catch(console.error);
                    break;
                case "❌":
                    reaction.users.remove(user).catch(console.error);
