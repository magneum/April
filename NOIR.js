...
const commandFiles = readdirSync(join(__dirname, "ɴᴏɪʀ_ʜᴏᴍᴇ")).filter((file) => file.endsWith(".js"));
for (const file of commandFiles) {
  const command = require(join(__dirname, "ɴᴏɪʀ_ʜᴏᴍᴇ", `${file}`));
...
NOIR.on("message", async (message) => {
  ...
  if (timestamps.has(message.author.id)) {
    ...
    if (now < expirationTime) {
      ...
      return message.reply(pnoir.__mf("common.ɴᴏɪʀ_cool_down", { time: timeLeft.toFixed(1), name: command.name }));
    }
  }
  timestamps.set(message.author.id, now);
  setTimeout(() => timestamps.delete(message.author.id), cooldownAmount);
  try { command.execute(message, args); }
  catch (error) { console.error(error); message.reply(pnoir.__("common.ɴᴏɪʀ_error_Command")).catch(console.error); }
});
/**
 * 
 * 
